...
void initRender(OpenGL *ogl)
{
 ...
 void DrawFigure(double Figure[8][3]); //с нормалями
double* CalculateNorm(double v1[], double v2[], double v3[]);

void DrawPolygon(double v1[], double v2[], double v3[], double v4[], double v5[],int flag=0)
{
...
void DrawCutted(double v1[], double v2[], double v3[])
{
	...
	double tmp1[3];
	double tmp2[3];
	double O[] = { 3,4,(v1[2] + v2[2]) / 2 }; //координаты посчитанные по системе уравнений посчитанный на основе системцы руавнений
	double r = 3.162277660168379;//радиус посчитанный на основе системцы руавнений
	tmp1[2] = (v1[2] + v2[2]) / 2;
	tmp2[2] = (v1[2] + v2[2]) / 2;
	for(int i=1; i<=n;i++)
	{
...
void DrawFigure(double Figure[8][3])
{
	...
	glBegin(GL_POLYGON);
	glTexCoord2d(2.0 / 11.0, 2.0 / 11.0);
	glVertex3dv(Figure[0]);
	glTexCoord2d(8.0 / 11.0, 2.0 / 11.0);
	glVertex3dv(Figure[6]);
	glTexCoord2d(8.0 / 11.0, 4.0 / 11.0);
	glVertex3dv(Figure[7]);
	glTexCoord2d(6.0 / 11.0, 5.0 / 11.0);
	glVertex3dv(Figure[2]);
	glTexCoord2d(3.0 / 11.0, 4.0 / 11.0);
	glVertex3dv(Figure[1]);
	glEnd();
	glBegin(GL_TRIANGLES);
	glTexCoord2d(6.0 / 11.0, 5.0 / 11.0);
	glVertex3dv(Figure[2]);
	glTexCoord2d(8.0 / 11.0, 6.0 / 11.0);
	glVertex3dv(Figure[5]);
	glTexCoord2d(8.0 / 11.0, 4.0 / 11.0);
	glVertex3dv(Figure[7]);
	glEnd();
	glBegin(GL_TRIANGLES);
	...
	glTexCoord2d(8.0 / 11.0, 6.0 / 11.0);
	glVertex3dv(Figure[5]);
	glTexCoord2d(4.0 / 11.0, 10.0 / 11.0);
	glVertex3dv(Figure[4]);
	glEnd();
	DrawCutted(Figure[2], Figure[3], Figure[4]);
}
...
void Render(OpenGL *ogl)
{
    ...
    double Figure[8][3] = { { 2,2,0 },{ 3,4,0 },{ 6,5,0 },{ 2,7,0 },{ 4,10,0 },{ 8,6,0 },{ 8,2,0 },{ 8,5,0 } };   //с нормалями
	double M[3] = { 4,7,0 };
	glBindTexture(GL_TEXTURE_2D, texId);
	DrawFigure(Figure);
